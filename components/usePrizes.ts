...
const CACHE_VERSION = "v2_prizes"; // キャッシュのバージョンを更新

type CachePayload = { v: string; savedAt: number; data: Prize[] };
...
function makeCacheKey(gasUrl: string) {
  return `prizes_cache_${CACHE_VERSION}_${encodeURIComponent(gasUrl)}`;
}

export function usePrizes(options?: { forceRefresh?: boolean }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const url = `${resolvedGasUrl}?type=prizes`;
        const res = await fetch(url, { signal: controller.signal, cache: "no-store" });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);

        const data = await res.json();
        
        // ★最大の修正ポイント：データが { prizes: [...] } だった場合に対応
        const arr: Prize[] = Array.isArray(data) ? data : (data.prizes || []);
